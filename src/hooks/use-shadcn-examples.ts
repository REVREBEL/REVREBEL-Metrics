...
import type { ShadcnExamplesIndex } from "@/lib/workshop/shadcn-examples-index";
import { validateShadcnExamplesIndex } from "@/lib/workshop/shadcn-examples";
...
export function useShadcnExamples(): UseShadcnExamplesResult {
  ...
  const load = useCallback(async (ignore?: { current: boolean }) => {
    ...
    try {
      const response = await fetch("/workshop/shadcn-examples.json", {
        cache: "no-store",
      });
